...
virt-cat -a "$img" /etc/appliance/build.env > "out/$name-build.env"
virt-cat -a "$img" /etc/appliance/packages.txt > "out/$name-packages.txt"
...
jq -n \
  ...
  --arg img "$img_sha" --arg zst "$zst_sha" --arg qcow "$qcow_sha" --arg iso "$iso_sha" \
  --argjson base "$source_json" \
  '{schema:1,variant:$variant,build_time:$build_time,base:$base,versions:{frr:$frr,vpp:$vpp},git:{commit:$commit},ci:{pipeline:$pipeline,runner:$runner},artifacts:{img:{sha256:$img},img_zst:{sha256:$zst},qcow2:{sha256:$qcow},installer_iso:{sha256:$iso}}}' \
...
jq -n \
  ...
  --arg img "$img_sha" --arg zst "$zst_sha" --arg qcow "$qcow_sha" --arg iso "$iso_sha" \
  '{_type:"https://in-toto.io/Statement/v1",subject:[{name:($subject+".img"),digest:{sha256:$img}},{name:($subject+".img.zst"),digest:{sha256:$zst}},{name:($subject+".qcow2"),digest:{sha256:$qcow}},{name:($subject+"-installer.iso"),digest:{sha256:$iso}}],predicateType:"https://slsa.dev/provenance/v1",predicate:{buildDefinition:{buildType:"https://gitlab.com/frr-appliance/qemu-debian-installer@v1",externalParameters:{source_repository:$repo,source_commit:$commit},resolvedDependencies:[{uri:$base_url,digest:{sha512:$base_sha512}}]},runDetails:{builder:{id:$pipeline},metadata:{invocationId:$pipeline}}}}' \
