#!/usr/bin/env bash
set -euo pipefail
suite=${1:?}

install -d -m 0755 /usr/share/keyrings
curl --fail --location --retry 4 --retry-all-errors --proto '=https' --tlsv1.2 \
  https://deb.frrouting.org/frr/keys.gpg -o /tmp/frrouting.gpg

# Trust only the repository signing keys currently published by FRRouting.
# The upstream bundle contains historical keys too; import it into a temporary
# keyring, verify the required fingerprints are present, then export only the
# reviewed keys into APT's signed-by keyring.
allowed=(
  4A56C7738BB3F81595A805D2A832769908F13ED1
  3D9968AC9AE7BE1169288DDB1FD5839895F57FDA
  BBC9ACA9D13025A2C186FF7F741E92A1F6E3975B
)

gpg --homedir "${GNUPGHOME:?}" --batch --quiet --import /tmp/frrouting.gpg
mapfile -t fetched < <(gpg --homedir "$GNUPGHOME" --batch --with-colons --list-keys | awk -F: '
  $1=="pub" {want=1; next}
  $1=="fpr" && want {print toupper($10); want=0}
')
[[ ${#fetched[@]} -gt 0 ]] || { echo "FRR keyring contains no primary-key fingerprints" >&2; exit 1; }

for want in "${allowed[@]}"; do
  found=0
  for got in "${fetched[@]}"; do
    [[ "$got" == "$want" ]] && found=1
  done
  [[ $found -eq 1 ]] || { echo "Required FRR signing key missing: $want" >&2; exit 1; }
done

# Do not make historical/unreviewed keys from the upstream bundle trusted by APT.
gpg --homedir "$GNUPGHOME" --batch --export "${allowed[@]}" > /usr/share/keyrings/frrouting.gpg
[[ -s /usr/share/keyrings/frrouting.gpg ]] || { echo "Filtered FRR keyring is empty" >&2; exit 1; }
chmod 0644 /usr/share/keyrings/frrouting.gpg

cat > /etc/apt/sources.list.d/frr.list <<APT
deb [signed-by=/usr/share/keyrings/frrouting.gpg] https://deb.frrouting.org/frr $suite frr-stable
APT
apt-get update
apt-get install -y --no-install-recommends frr frr-pythontools frr-rpki-rtrlib frr-snmp
